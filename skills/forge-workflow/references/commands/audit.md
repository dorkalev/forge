...
3. **Spec Alignment**: `git diff origin/origin/staging..HEAD --name-only` vs spec documentation. PASS/WARN (N files not in spec).
...
8. **Secrets Check**: `git diff origin/origin/staging..HEAD` — scan for API keys, tokens, passwords, .env values. PASS/FAIL.
