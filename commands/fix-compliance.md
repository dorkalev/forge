...
/fix-compliance --tickets "ENG-123,ENG-456"
```
...
- worker/scripts/migrate_data.py
...
```bash
/fix-compliance --files "web/api.py,worker/process.py"
```
...
```
SOC2 check failed: ENG-335 has no code changes (from merge commit)

/fix-compliance
→ Detects ENG-335 came from merging staging
...
→ User: "Add to ENG-407"
→ Adds to Key Changes with ENG-407
→ Updates PR
```
