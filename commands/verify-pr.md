...
| Ticket | Title | Status |
|--------|-------|--------|
| [BOL-123](url) | Main Feature | Done |
| [BOL-124](url) | Related Fix | Done |
...
Derive feature name from branch (e.g., `bol-123-fix-login` -> look for `specs/fix-login.md` or `specs/bol-123-fix-login.md`).
...
| Ticket | Title | Status |
|--------|-------|--------|
| [BOL-123](https://linear.app/boltx/issue/BOL-123) | Main Feature | Done |
```
...
```bash
curl -s -o /dev/null -w "%{http_code}" "https://linear.app/boltx/issue/BOL-123"
```
