...
```
/release-media --tickets ENG-123,ENG-456 [options]
/release-media --tickets ENG-123 --base main [options]
```
...
- `--tickets <ids>` - Comma-separated ticket IDs (e.g., ENG-123,ENG-456)
- `--base <branch>` - Base branch to diff against (default: staging, or main)
- `--folders <paths>` - Limit diff to specific folders (e.g., web/,portal/,infra/)
...
- Spell out acronyms that should be pronounced (e.g., "Acme X" not "ACMEX")
...
### From Tickets
```
/release-media --tickets ENG-396 --title "January Release" --image web/static/og-image.png
```

```
/release-media --tickets ENG-123,ENG-124,ENG-125 --base main --folders web/,api/
```
