...
### Select Issue
**If argument is an issue ID** (e.g., `ENG-420`), fetch via Linear MCP, skip to Create Branch.
...
- Otherwise → this is a new issue description. Optionally improve spec with AI first. Then call `linear_create_issue` and **extract the identifier from the API response** (e.g., `ENG-123`). Use ONLY this returned identifier for all subsequent steps — NEVER fabricate or guess an issue ID.
