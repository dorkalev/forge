...
Present comprehensive summary: commits count, PRs included (table with compliance status), Linear tickets (table with status), key changes by area, compliance checklist. Group changes by area (Web, Worker, Infrastructure, etc.).
...
{grouped change summary from Phase 3 — e.g., Web, Worker, Infrastructure}
